		...
		<form id="#">
			<ul class="clearfix">
				...
				<li class="searchTerms">
					...
					<input class="inputField" type="text" name="likeTrackerId" placeholder="" value="${SEARCH_MAP.likeTrackerId }" />
				</li>				
				<li class="searchTerms">
								...
								<tr>
									<sec:authorize access="hasRole('system_admin')">
										<td class="txC ft12" data-checked="">
											<input type="checkbox" name="trackerId" value="${item.tracker_id }" />
									...
									<c:choose>
										<c:when test="${LOGIN_ROLE == 'system_admin'}">
											<td class="point num" data-detailid="${item.tracker_id }">${item.tracker_id }</td>
										</c:when>
										<c:otherwise>
											<td class="num">${item.tracker_id }</td>
										</c:otherwise>
									</c:choose>											
...
<script src="${res }/tracker/js/trackerList.js"></script>